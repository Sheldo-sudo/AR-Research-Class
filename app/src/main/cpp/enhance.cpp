...
Mat perfectReflectorBalance(const Mat& channel, double percentile = 0.1) {
    Mat channelFloat;
    channel.convertTo(channelFloat, CV_32F);

    Mat flat = channelFloat.reshape(1, 1); // 转为单行
    cv::sort(flat, flat, SORT_EVERY_ROW + SORT_ASCENDING);

    int refIndex = cvRound(flat.cols * (1.0 - percentile));
    refIndex = std::min(std::max(refIndex, 0), flat.cols - 1);

    double refValue = flat.at<float>(0, refIndex);

    if (refValue <= 0) {
        return channel.clone();
    }

    Mat balanced = channelFloat * (255.0 / refValue);
    threshold(balanced, balanced, 255.0, 255.0, THRESH_TRUNC);
...
void MSRCR(Mat& inputImage, double dynamic = 4.0, double alpha = 20.0, double beta = 12.0) {
    ...
    parallel_for_(Range(0, 3), [&](const Range& range) {
        for (int ch = range.start; ch < range.end; ++ch) {
            ...
            //完美反射取30%
            //Mat perfectReflectorAdjusted = perfectReflectorBalance(originalChannel, 0.05);
            //double weight = 0.3;
            //retinexChannel = weight * perfectReflectorAdjusted + (1.0 - weight) * retinexChannel;
    ...
    // 轻微提升饱和度
    //hsvChannels[1] = hsvChannels[1] * 1.1;
    //threshold(hsvChannels[1], hsvChannels[1], 255, 255, THRESH_TRUNC);
